...
async function getAvb(user, model, color) {
  ...
  skus.forEach((sku, i) => {
      ...
      svgWrapper.addEventListener( 'click', async (e) => {
        ...
        let pic = await fetch(`api/image/${sku.year}/${sku.season}/${sku.model}`)
            .then(res => res.text())
            .then(url => img.src = url)
      })
      ...
      skuElement.addEventListener('click', async (event) => {
        ...
        if (!skuElement.classList.contains('fetched')) {
          ...
          for (var y = 0; y < sku.sizesForRequests.length; y++) {
            let shopsObject = fetch(`/api/${sku.year}/${sku.season}/${sku.model}/${sku.color}/${sku.sizesForRequests[y]}`)
              .then(res => res.json());
      ...
      let priceGetter = async () => {
        let res = await fetch(`/api/price/${sku.year}/${sku.season}/${sku.model}`)
