...
async function getPrice(cookie, year, season, model) {
  try {
    const html = await fetch('https://websmart.brunellocucinelli.it/bcweb/WWEBORD07R.pgm', {
      credentials: 'include',
      headers: {
        cookie,
      },
...
async function getToBeReceived(cookie, model, color) {
  ...
  const url = `https://websmart.brunellocucinelli.it/bcweb/WRTIRIO02R.pgm?TASK=dettaglio&BGACODICE=${model}&COLORE=${color}&TIPO=DARIC`;
  ...
  const thRow = $d('.mainlist:nth-child(2) thead tr th');
  const sizesRow = $d('.mainlist tbody tr:nth-child(1) td');

  sizesRow.each((i) => {
    const th = $d(thRow[i]).text();
    const size = $d(sizesRow[i]).text();
...
async function getReceivables(cookie, year, season, model, color) {
  const html = await fetch('https://websmart.brunellocucinelli.it/bcweb/WRTIRIO02R.pgm', {
    credentials: 'include',
    headers: {
      cookie,
    },
  ...
  const total = $('tr.altcol1:nth-child(2) > td:nth-child(17)').text();
...
async function getShops(cookie, year, season, model, color, size) {
  ...
  const fetchUrl = 'https:/' + `/websmart.brunellocucinelli.it/bcweb/WRTICMO10R.pgm?TASK=dett&BCRSTGANN=${year}&BCRSTGSIG=${season}&BCRMODELL=${model}&BCRCOLORE=${color}&idx_taglia=${size}`;
  ...
  await fetch(fetchUrl, {
    ...
    .then((text) => {
      ...
      const sizeLabel = $r('table.mainlist:nth-child(1) > thead:nth-child(1) > tr:nth-child(4) > th:nth-child(2)').text().trim();
      sizes[size] = {};
      sizes[size][sizeLabel] = [];

      const shops = $r('table.mainlist:nth-child(2) > tbody:nth-child(2) > tr > td:nth-child(2)');
    ...
    const avb = async (i) => {
      ...
      if ($element.find($('th')).length > 0) {
        sizeRow = $(rows[i]).text().replace(/(\r\n|\n|\r)/gm, '').trim()
          .split(' ')
          .filter((size) => size != '' && size != 'Colore' && size != 'Stagione' && size != 'Modello');
      }

      const sku = {};
      // If theres an actual result in the row (check for td)
      if ($element.find($('td')).length > 0) {
        const tds = Object.values($element.find($('td')));
...
async function getDhl(tracking) {
  const awb = await fetch(`https://www.dhl.com/shipmentTracking?AWB=${tracking}&countryCode=g0&languageCode=en&_=1607198868925`, {
    credentials: 'include',
    headers: {
      'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:81.0) Gecko/20100101 Firefox/81.0',
